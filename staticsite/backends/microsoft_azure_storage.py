...
BlobServiceClient = check_publisher_dependencies('staticsite.backends.azure_storage', 'azure.storage.blob', 'BlobServiceClient')
BlobClient = check_publisher_dependencies('staticsite.backends.azure_storage', 'azure.storage.blob', 'BlobClient')
ContentSettings = check_publisher_dependencies('staticsite.backends.azure_storage', 'azure.storage.blob', 'ContentSettings')
...
class AzureBlobStorateBackend(PublisherBackendBase):
    ...
    def _get_container(self):
        return self.d['connection'].get_container_client(container=self.account_container())
    ...
    def authenticate(
        ...
        self.d['connection'] = BlobServiceClient.from_connection_string(conn_str=self.connection_string())
        return True
    ...
    def _check_file(
        ...
        for i in range(self.RETRY_ATTEMPTS):
            ...
            if local_hash == remote_hash:
                return True
        raise StaticSitePublishError(f'Failed to upload local file "{local_name}" blob to Azure container at '
