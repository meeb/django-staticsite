...
def check_publisher_dependencies(
    ...
    try:
        ...
        stderr.write(f'Static site backend "{required_by}" requires module "{module_name}" to be installed')
        raise
...
def get_backend(
    ...
    with warnings.catch_warnings():
        ...
        try:
            ...
            stderr.write(f'Static site backend "{engine_name}" not found or failed to import: {e}')
            raise
...
class PublisherBackendBase(object):
    ...
    def _validate_options(
        ...
        for o in self.REQUIRED_OPTIONS:
            if o not in self.options:
                raise StaticSitePublishError(f'Missing required settings value for the specified '
    ...
    def detect_local_file_mimetype(
        ...
        try:
            ...
            raise StaticSitePublishError(f'Failed to guess mimetype for {local_name}: {e}') from e
    ...
    def generate_remote_url(
        ...
        if not local_name.is_relative_to(self.source_dir):
            raise StaticSitePublishError(f'Local static site file "{local_name}" is not '
                                         f'in source dir "{self.source_dir}"')
        remote_uri = self.remote_url_parts.path + self.remote_path(local_name)
        return urlunsplit((self.remote_url_parts.scheme, self.remote_url_parts.netloc, remote_uri, '', ''))
    ...
    def check_file(
        self,
        local_name: Path | None,
        url: str
    ) -> bool:
        if not self._file_exists(local_name):
            raise StaticSitePublishError(f'Local static site file does not exist: {local_name}')
